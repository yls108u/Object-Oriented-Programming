...
//copy
// this packet is used to tell the hi msg
...
class node {
    ...
    public:
        ...
        map<unsigned int,bool> my_neighbors; //map to store each node's neighbors!!
...
//copy
void add_H_initial_event (unsigned int src, unsigned int dst, unsigned t = 0, string msg = "default"){
...
//you can get the dest between two node ids
...
void GR_node::recv_handler (packet *p){
    ...
    //map<unsigned int, bool> my_neighbors;
    ...
    if (p->type() == "Hello_packet"){ //recv Hello_packet
        ...
        if((my_neighbors.find(nb_id)) != (my_neighbors.end()) || nb_id != getNodeID()) {
            if(nb_id != getNodeID()){
                my_neighbors.insert(pair<unsigned int, bool> (nb_id, 1)); //store neighbor in map
            }
            //cout<<"already Hello, drop it"<<endl;
            return ;
        }

        p2->getHeader()->setPreID ( getNodeID() );
        p2->getHeader()->setNexID ( nex_id );
        send_handler (p2);
    }

    if (p->type() == "GR_packet" /*&& !hi */){ //recv GR_packet
        ...
        double now_dst = GET_dst(dst_id, now_id);

        //cout<<"now id = "<<getNodeID()<<"\t"<<"maxsize = "<<my_neighbors.size()<<endl;
        for(iter = my_neighbors.begin(); iter != my_neighbors.end(); iter++){ //use map to get neighbors
            double cur_dst = GET_dst(dst_id, iter->first);
            if(now_dst > cur_dst){
                nex_id = iter->first;
                now_dst = cur_dst;
            }
        ...
        if(nex_id == BROCAST_ID){ //current node is the closest
            //cout<<"now_id:"<<now_id<<"\t"<<"nex_id:"<<nex_id<<endl;
            return ;
        }
...
int main()
{
    ...
    for (unsigned int i = 0; i < node_cnt; i ++){
        ...
        nd->setX(x);
        nd->setY(y);
        add_H_initial_event(id, BROCAST_ID, BR_time); //add event to pass Hello messages
    }
